...
-- 2022-05-31,PBaculi,Revised File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_PBaculi')
	 Begin 
	  Alter Database [Assignment07DB_PBaculi] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_PBaculi;
	 End
	Create Database Assignment07DB_PBaculi;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_PBaculi;
...
--go
--SELECT 
-- [UnitPriceUSD] = FORMAT (UnitPrice, 'c', 'en-US')
-- from vProducts;
--go
--Select 
-- ProductName
...
-- vC.CategoryName
-- ,vP.ProductName
-- ,[UnitPriceUSD] = FORMAT (vP.UnitPrice, 'c', 'en-US')
...
--       ,[InventoryDate] = DATENAME("month",vI.InventoryDate) + ', ' + DATENAME("year",vI.InventoryDate)
...
-- <Put Your Code Here> --
/*below created from code in question 3*/
go
Create --or Alter
...
--  from vInventories
--Group by ProductID, InventoryDate
--go
--Select 
-- vC.CategoryName
-- ,[InventoryDate] = DATENAME("month",vI.InventoryDate) + ', ' + DATENAME("year",vI.InventoryDate)
...
--go
Create Function dbo.fProductInventoriesWithPreviousMonthCountsWithKPIs (@Value1 integer)
 Returns Table
 As
  Return (
     Select TOP 1000000
